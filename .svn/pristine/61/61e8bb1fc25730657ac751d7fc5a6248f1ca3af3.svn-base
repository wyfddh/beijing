						...
						<div class="row " id="detailCulHtml" style="padding: 12px;">
							<form method="post" action="#" class="registerform" id="cgform" >
								...
								<div class="col-xs-12 col-md-12" style="margin-bottom: 1px; padding-right: 0px; padding-left: 0px;">
									<div class="profile-user-info profile-user-info-striped" style="margin: 0 1px; border: 1px solid #CCCCCC;">
										<div class="profile-info-row">
											<div class="profile-info-name"><span class="rad">*</span>查询模式</div>
											<div class="profile-info-value">
												<select style="width:100%;" datatype="/^\s*$/|*" id="queryMode" name="queryMode">
												  <option value="single" <c:if test="${bean.queryMode =='single'}">selected</c:if>>单一查询</option>  
												  <option value="group" <c:if test="${bean.queryMode =='group'}">selected</c:if>>组合查询</option>  
												  </select>
											</div>
										</div>
									</div>
								...
								<div class="col-xs-12 col-md-12" style="margin-bottom: 1px; padding-right: 0px; padding-left: 0px;">
									<div class="profile-user-info profile-user-info-striped" style="margin: 0 1px; border: 1px solid #CCCCCC;">
										<div class="profile-info-row">
											<div class="profile-info-name"><span class="rad">*</span>查询方式</div>
											<div class="profile-info-value">
												<select style="width:100%;" datatype="/^\s*$/|*" id="queryType" name="queryType">
												  <option value="like" <c:if test="${bean.queryType =='like'}">selected</c:if>>模糊查询</option>  
												  <option value="l_like" <c:if test="${bean.queryType =='l_like'}">selected</c:if>>左模糊查询</option>
												  <option value="r_like" <c:if test="${bean.queryType =='r_like'}">selected</c:if>>右模糊查询</option>
												  <option value="equal" <c:if test="${bean.queryType =='equal'}">selected</c:if>>相等</option>
												 </select>
											</div>
										</div>
									</div>
