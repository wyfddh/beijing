package com.design.core.dao.jdbc;
...
import com.design.core.common.listener.DBTypeUtil;
...
public class JdbcDao extends SimpleJdbcTemplate{
	...
	public static String jeecgCreatePageSql(String sql, int page, int rows){
		...
		sqlParam[1] = beginNum+"";
		sqlParam[2] = rows+"";
		if(DBTypeUtil.getDBType().indexOf(DATABSE_TYPE_MYSQL)!=-1){
			sql = MessageFormat.format(MYSQL_SQL, sqlParam);
		}else if(DBTypeUtil.getDBType().indexOf(DATABSE_TYPE_POSTGRE)!=-1){
			...
			if(DBTypeUtil.getDBType().indexOf(DATABSE_TYPE_ORACLE)!=-1) {
				sql = MessageFormat.format(ORACLE_SQL, sqlParam);
			} else if(DBTypeUtil.getDBType().indexOf(DATABSE_TYPE_SQLSERVER)!=-1) {
