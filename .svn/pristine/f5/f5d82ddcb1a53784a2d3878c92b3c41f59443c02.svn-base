...
<form class="layui-form" id="newForm">
	...
	<div class="layui-col-xs8" style="padding-right: 20px;">
		<div class="layui-card-header layui-row layui-col-xs12" id="div_title">
			<label><b>${article.title }</b></label>
		</div>
		<div class="layui-card-body layui-row layui-col-xs12" style="overflow:auto;" id="div_content">
			${article.content }
		</div>
	</div>
	<div class="layui-col-xs4" style="padding-left: 20px;">
		...
		<div class="layui-form-item layui-row layui-col-xs12">
			<c:if test="${not empty article.titleImageSrc }" var="imgIsNotNull">
				<div id="showImg" style="width: 95%;height: 200px;margin-bottom: 10px;"><a href="javascript:void(0);"><img id="titleImgShow" style="max-height: 100%;max-width: 100%;" src="${article.titleImageSrc }" ></a></div>
			</c:if>
		</div> --%>
		...
		<div class="layui-form-item layui-row layui-col-xs12" style="vertical-align:middle;">
			<input type="text" class="layui-input inputText" placeholder="" lay-verify="required" value="${article.author }" name="author" id="author" style="width:95%;" readonly="readonly">
		</div>
		...
		<div class="layui-form-item layui-row layui-col-xs12" style="vertical-align:middle;">
			<textarea type="text" class="layui-textarea textarea" placeholder="" value="${article.description }" name="description" id="description" row="3" style="width:95%;resize:none;" readonly="readonly">${article.description }</textarea>
		</div>
...
$(function(){
	var title = document.getElementById("div_title").offsetHeight;
	$("#div_content").css("height", ($(window).height()-title-30)+"px");
});
//接收单位
var sel_receive = [];
//附件
