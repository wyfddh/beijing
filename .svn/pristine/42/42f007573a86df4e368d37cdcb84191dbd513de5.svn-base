layui.use(['form','layer','table','laytpl','element'],function(){
    ...
    var tableIns = table.render({
        ...
        url : projectName + '/collections/dataList.do', 
        request:{
...
//            {field: 'gsSource', title: '文物来源', align:'center',minWidth:50},  
    ...
    function getFossilTable() {
    	tableIns1 = table.render({
            ...
            url : projectName + '/collectionsFossil/dataList.do', 
            request:{
            	pageName: 'currentPage',
            	limitName: 'size'
            },
	...
	$(".daochuList li").off().on('click', function (e) {
	      ...
	      if ($(this).index() == 0) {
	    	  ...
	    	  if(data != null && data.length>0){
	    		  ...
	    		  if(count > 0){
	    			  if(newData.length>0){
//	    				  layer.msg('操作成功,已自动剔除无效选项', {icon: 2});  
...
//	    				  layer.msg('操作成功,已自动剔除无效选项', {icon: 1});  
	        ...
	        return false;
	    	  ...
	    	  if(data != null && data.length>0){
	    		  ...
	    		  if(count > 0){
	    			  if(isData.length>0 || gjData.length>0 || allData.length>0){
//	    				  layer.msg('操作成功,已自动剔除无效选项', {icon: 1});  
	    				  teachOpenSetting(gjData, isData, allData, type);
	    			  }else{
	    				  layer.msg('操作失败,所选项均为无效选项', {icon: 2});  
	    			  }
	    		  }else{
...
//				top.layer.close(indexpub);
	...
	function teachOpenSetting(gjData, isData, allData, type){
		layer.msg("等待中", {
			icon:16,
			shade:[0.5],
			time:false  //取消自动关闭
		})
		var is1 = gjData.length==0?true:openSetting1(gjData,"1",type);
		if(is1){
			var is2 = isData.length==0?true:openSetting1(isData,"2",type);
			if(is2){
				var is3 = allData.length==0?true:openSetting1(allData,"3",type);
				if(is3){
	...
	function openSetting1(data,info,type){
		var isTrue = false;
		if(data.length == 0){
//			isTrue =  false;
		}
