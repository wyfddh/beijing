...
<body class="childrenBody">
					...
					<ul class="layui-colla-content layui-nav layui-nav-tree" style="height: 100%;width: 100%;" >
						<c:forEach items="${myQuery}" var ="li" varStatus="status">
							<li class="layui-nav-item item1">
								<a href="javascript:query('${li.queryTerms}','${li.queryOrgs}','${li.sqlstr}');"><cite>${li.queryName}</cite></a>
							</li>
						</c:forEach>
					</ul>
				</div>
	...
	<script type="text/javascript">
		...
		function zTreeBeforeCheck(treeId, treeNode) {
			 if(treeNode.isParent){
				treeNode.chkDisabled = true;	
				 ...
				 if(curPid == ""){
					 curPid = treeNode.pId;
				 }else if(curPid == treeNode.pId){
					 //同一父节点下面 不用操作
				 }else{
		            	var treeObj = $.fn.zTree.getZTreeObj("treeDemo");					
						var flag = 0;//用来判断 点了取消
						layer.open({
							  type: 0, 
							  content: '自定义明细数据查询时，请选取同一个分类下的字段，若您选择继续，将使用该字段替换当前查询字段!',
							  btn:['继续', '取消'],
							  yes: function (index, layero) {
			                        ...
			                        $.ajax({
			                            ...
			                            data:{"queryTerms":queryTerms,"queryOrgs":queryOrgs,"type":"1","curPid":curPid},
		 ...
		 function onCheck(){	
			...
			tableCols.push({field: 'd_col0',width:200,title: "组织名称",sort: true, fixed: 'left'});
		...
		function backCheck(queryTerms){
			...
			//每次点击 清空数组
			tableCols.splice(0,tableCols.length);
			tableCols.push({field: 'd_col0',title: "组织名称", sort: true, fixed: 'left'})
			for(var i=0;i<nodes.length;i++){
				var field = nodes[i]["uniqueName"];
				var name = nodes[i]["name"];
				tableCols.push({field: field,title: name,sort: true})
	    ...
	    $(document).ready(function () {
	     ...
	     function initNode() {
	        $.ajax({
	            type: 'GET',
	            url:  appName+ "/cmsSubject/list.do?Pid=3",
	            dataType: "text",
	            success: function (data) {
	                var obj = eval('(' + data + ')');
	                zNodes = obj.data;
	...
	function query(Terms,Orgs,Sql){	
		...
		$.ajax({
            type: 'post',
            url:  appName+ "/museuminfo/query.do",
            data:{"queryTerms":Terms,"queryOrgs":Orgs,"type":"1","sqlstr":sqlstr},
            success: function (data) {
                 //展示已知数据
             	 tableData = data.data;
                 //参数带过去 回显选中左边树
	...
	function openSave(){
		var url = appName+ "/museuminfo/openSave.do?queryTerms="+queryTerms+"&queryOrgs="+queryOrgs+"&type=1"+"&curPid="+curPid;
	    layer.open({
	        type: 2,
	        title: '保存查询',
	        shadeClose: true,
	...
	　　	queryArr = [];
	　　	for(var i=0;i<nodes.length;i++){
	　　		queryTerms += nodes[i].uniqueName+",";
	　　		queryArr.push(nodes[i].uniqueName+","+nodes[i].name);
	　　	}
