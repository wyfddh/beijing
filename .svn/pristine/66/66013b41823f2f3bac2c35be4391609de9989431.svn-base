...
<script type="text/javascript">
	var EntityList = avalon.define({
		...
		pageNum: [{"num": 10}],
		queryPageCodeRdm : "cgformBussController.do?goQueryMain&bussCode=${bussCode}",//查询条件页面
		//查询条件页面加载完成事件
		...
		GetLIST: function(rFData){
			...
			var baseUrl = "cgformBussController.do?getCgformBuss";
		...
		showMTabHtml: function(data){
			...
			if(data.MainList!=null && data.MainList.length>0){
				...
				divhtml += "<th style='border:1px solid #CBCDCE;'>业务类型名称</th>";
				divhtml += "<th style='border:1px solid #CBCDCE;'>菜单名称</th>";
				...
				for(var j=0;j<data.MainList.length;j++){
					...
					divhtml += "<td style='border:1px solid #CBCDCE;text-align:center;'>"+data.MainList[j].buss_name+"</td>";
					divhtml += "<td style='border:1px solid #CBCDCE;'>"+data.MainList[j].menu_name+"</td>";
		...
		goAdd: function(){
			jumpPage("cgformBussController.do?goAdd","新增业务类型");
		},
		goUpdate: function(){
			...
			jumpPage("cgformBussController.do?goUpdate&id="+mxids,"编辑业务类型");
		},
		goRelease: function(){
			var mxids = getRowId("main");
			if(mxids==""){
