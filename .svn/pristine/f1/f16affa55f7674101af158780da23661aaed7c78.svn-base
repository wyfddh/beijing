        ...
        <input type="hidden" id="level" name="level" value="${level}">
	            <div class="layui-inline">
	                <div class="layui-input-inline">
	                    <input type="text" class="layui-input searchVal" placeholder="请输入活动标题/内容"  name="key" id="key"/>
	                </div>	
	                <div class="layui-input-inline">
	            ...
	            <div class="layui-inline a2">
                	<a class="layui-btn" onclick="add()">新增亲子活动</a>
            	</div>	            
    		</form> 
  	</blockquote>  	           
     <div>
...
$(function(){
   ...
   tableIns = table.render({
	    elem: '#activityList',
	    url:'<%=request.getContextPath() %>/activityAdmin/getActivityList.do?type=1',
	    request:{
	    ...
	    cols: [[
	      ...
	      ,{title: '操作',width:210,style:"height:70px;",align:"center",fixed:'right',templet:function(d){
          	var html = ''; 
...
function add(){
	var index = layui.layer.open({
        ...
        title: '新增活动',
        area: ['70%', '800px'],
        content: "<%=request.getContextPath() %>/activityAdmin/toActivityAdd.do",
...
function edit(id){
    var index = layui.layer.open({
        type: 2,
        title: '编辑活动',
        content: "<%=request.getContextPath() %>/activityAdmin/toActivityUpdate.do?id="+id,
        area: ['70%', '800px'],
...
window.view = function(id){
	window.open("<%=webIp %>html/qinzihuodong.html?bg="+id);
}
//时间戳转时间格式
