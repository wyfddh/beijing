layui.use(['form','layer','table','laytpl','laydate'],function(){
    ...
    var tableIns = table.render({
        ...
        cols : [[
            ...
            {field: 'firstKindName', title: '一级分类', minWidth:200, align:'center'},
            {field: 'secondKindName', title: '二级分类', align:'center',minWidth:100},
            {field: 'publisher', title: '发布单位', align:'center',minWidth:100},
            {field: 'publishYear', title: '发布年份', align:'center',minWidth:100},
            {field: 'code', title: '条款号', align:'center',minWidth:150},
            {title: '操作', minWidth:175, templet:'#legalListBar',fixed:"right",align:"center"}
        ]],
        done:function(){
        ...
        if(layEvent === 'edit'){ //编辑
            ...
            var showIndex = layui.layer.open({
                ...
                success : function(layero, showIndex){
                    ...
                    body.find("#textareaHide").val(data.content);
                    body.find("#firstKindHide").val(data.firstKindId);
                    body.find("#secondKindHide").val(data.secondKindId);
                    
                    body.find("#firstKind option[value='"+data.firstKindId+"']").attr("selected",true);
                   /* body.find("#secondKind option[value='"+edit.secondKindId+"']").attr("selected",true);*/
                    body.find("#publisher").val(data.publisher);
                    body.find("#publishYear").val(data.publishYear);
                    body.find("#code").val(data.code);
                    body.find("#divtext").text(data.realFileName);
                    body.find("#hideUrl").val(data.resultPath);
                    body.find("#legalId").val(data.id);
                    body.find("#attachmentId").val(data.attachmentId);
                    
                    //设置类型3为查看
                    body.find("#saveType").val("3");
                    //隐藏保存按钮
                    body.find("#saveBtn").hide(); 
                    //隐藏删除，显示下载
                    body.find("#delFile").hide(); 
                    body.find("#downFile").show(); 
                    //设置只读
