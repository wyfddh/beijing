...
<form class="layui-form" id="newForm">
	<input type="hidden" id="id" name="id" value="${wenchuang.id }">
	<input type="hidden" id="pictureId" name="pictureId" value="${wenchuang.pictureId }">
	<div class="layui-col-xs12" style="padding-right: 20px;padding-left: 20px;">
		<div class="layui-form-item layui-row layui-col-xs12">
			<input type="text" class="layui-input inputText" lay-verify="required" placeholder="标题" value="${wenchuang.productName }" id="productName" name="productName" style="width:70%;display: inline-block;">
		...
		<div class="layui-form-item layui-row layui-col-xs12">
			<c:if test="${not empty titleImg }" var="titleImgNotEmpty">
				<div id="showImg" style="width: 95%;height: 150px;margin-bottom: 10px;"><a href="javascript:void(0);" ><img id="titleImgShow" style="max-height: 100%;max-width: 100%;" src="${titleImg }" ></a></div>
			</c:if>
			<c:if test="${!titleImgNotEmpty }">
		...
		<div class="layui-form-item layui-row layui-col-xs12" style="vertical-align:middle;width:95%;">
			...
			<c:forEach items="${categoryList }" var="list">
				<option value="${list.id }" <c:if test="${wenchuang.categoryId == list.id }">selected</c:if> >${list.categoryName }</option>
			</c:forEach>
			</select>
		</div>
		...
		<div class="layui-form-item layui-row layui-col-xs12" style="vertical-align:middle;">
			<input type="text" class="layui-input inputText" lay-verify="required" placeholder="这里输入产品编号" value="${wenchuang.productNumber }" name="productNumber" id="productNumber" style="width:95%;">
		</div>
		...
		<div class="layui-form-item layui-row layui-col-xs12" style="vertical-align:middle;">
			<input type="text" class="layui-input inputText" lay-verify="required|ver_double" placeholder="这里输入产品价格（元）" value="${wenchuang.productPrice }" name="productPrice" id="productPrice" style="width:95%;">
		</div>
		...
		<div class="layui-form-item layui-row layui-col-xs12" style="vertical-align:middle;">
			<textarea type="text" class="layui-textarea textarea sy" onkeyup="checkLength(this);" placeholder="说点什么吧..." value="" name="designElements" id="designElements" row="3" style="width:95%;resize:none;">${wenchuang.designElements }</textarea>
