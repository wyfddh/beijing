 ...
 */
@Controller
@RequestMapping("/listPageGenNoticeController")
public class ListPageGenNoticeController  {
	/**
...
//	}
	...
	public List<LayuiCols> getListTitle(HttpServletRequest request,
		...
		List<CgformBussListEntity> mainList1 = new ArrayList<>();
		for (CgformBussListEntity cgformBussListEntity : mainList) {
...
//			||m.get("field_name").equals("org_name")
			...
			if("create_date".equals(cgformBussListEntity.getFieldName()) || "create_by".equals(cgformBussListEntity.getFieldName())
					||"create_name".equals(cgformBussListEntity.getFieldName()) || "org_id".equals(cgformBussListEntity.getFieldName())
					||"org_name".equals(cgformBussListEntity.getFieldName()) || "update_by".equals(cgformBussListEntity.getFieldName())
					||"update_name".equals(cgformBussListEntity.getFieldName()) || "update_date".equals(cgformBussListEntity.getFieldName())
					|| "status".equals(cgformBussListEntity.getFieldName()) || "buss_id".equals(cgformBussListEntity.getFieldName())) {
				
				//不显示这些列
			}else {
				mainList1.add(cgformBussListEntity);
			}
		}
		mainList = mainList1;
		...
		//提交时间
		LayuiCols layuiCols2=new LayuiCols();
		layuiCols2.setTitle("提交时间");
		layuiCols2.setAlign("left");
		layuiCols2.setWidth(200);
		layuiCols2.setField("submitTime");
		layuiCols2.setIsHide("N");
		layuiColsList.add(layuiCols2);
		//提交单位
		LayuiCols layuiCols1=new LayuiCols();
		layuiCols1.setTitle("单位");
		layuiCols1.setAlign("left");
		layuiCols1.setWidth(200);
		layuiCols1.setField("orgName");
		layuiCols1.setIsHide("N");
		layuiColsList.add(layuiCols1);
...
//					and field_name not in ('create_date','create_by','create_name','org_id','org_name','update_by','update_name','update_date')
		...
		ssq.append(" select b.write_time as submitTime, d.name as orgName, c.* from gov_notice a ");
		ssq.append(" left join gov_notice_org b on(a.id=b.notice_id) ");
		ssq.append(" left join ( ");
		ssq.append(" SELECT  "+(!"".equals(tempFields)?tempFields:fields));
		ssq.append(" FROM "+mainTableName+" ");
		ssq.append(" where 1=1 ");
		ssq.append(" ) c on(b.reportId=c.id) ");
		ssq.append(" left join mip_organization d on(d.id=b.org_id) ");
		ssq.append(" where b.receive_status = '4' ");
		ssq.append(" and a.id ='"+noticeId+"' ");
		if("2".equals(tSConfigform.getSaveType())){
...
//		}
		...
		if(aaq1!=null&&aaq1.size()>0){
			ssq.append("  ORDER BY  c.create_date desc ");
		}
...
//		}
		...
		String noticeId=oConvertUtils.getString(request.getParameter("noticeId"), "");
		String jumpPage=oConvertUtils.getString(request.getParameter("jumpPage"), "pagegen/notice_configformList");
