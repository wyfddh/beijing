		...
		<div class="layui-row">
			<div class="layui-col-md6 layui-col-md-offset3 ">
		        <div class="cardOne mt24">
		                <div class="title"><img src="<%=request.getContextPath() %>/back/images/xingxing.png" alt="">&nbsp;&nbsp;<span class="title_name">主要管理制度和质量管理体系</span></div>
...
layui.use(['form','layer','upload','laydate','element'],function(){
		...
		var uploadListIns = upload.render({
	        ...
	        ,choose: function(obj){
	            ...
	            obj.preview(function(index, file, result){
	                ...
	                "                                        <a href=\"javascript:void(0);\" class=\"fileName\" >"+file.name+"</a>" +
    ...
    form.on("submit(saveBase)",function(data){
        ...
        $.ajax({
            url:projectName+"/relicsBureau/saveCulturalManageFileList.do",
            data:json,
            type:'post',
            async:false,
	...
	function loadAttachments() {
	    ...
	    $.ajax({
	        ...
	        data:json,
	        async:false,
	        url:projectName+"/relicsBureau/getCulturalManageFileList.do",
	        success:function(result) {
	            if (result.success == 1) {
   ...
   function getAttachmentList(data) {
	    ...
	    for (var i=0;i<data.length;i++){
	    	if('${level}' == '1'){
		        ...
		        "                                        <a title=\"下载附件\" href=\"javascript:void(0);\" class=\"fileName\" data-path='"+data[i].attPath+"' data-name='"+data[i].attName+"'>"+data[i].attName+"</a>" +
