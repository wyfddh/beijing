	...
	<div>
	    ...
	    <div class="layui-row box-a" >
	    	...
	    	<div class="layui-col-md2 box-b">
	    		...
	    		<div class="box-d">
	    			<a class="ref-a" href="<%=request.getContextPath()%>/museumHouseBuilding/list.do?museumId=${result.museumId}&museumName=${result.museumName}">
	    			  <c:if test="${'1' eq level}">编辑资料</c:if>
	    			 <c:if test="${'1' ne level}">查看资料</c:if>
	    			</a>&nbsp;
	    	...
	    	<div class="layui-col-md2 box-b">
	    		...
	    		<div class="box-d">
	    			<a class="ref-a" href="<%=request.getContextPath()%>/museumDigitization/form.do?museumId=${result.museumId}">
	    			  <c:if test="${'1' eq level}">编辑资料</c:if>
	    			 <c:if test="${'1' ne level}">查看资料</c:if>
	    			</a>&nbsp;
	    ...
	    <div class="layui-row box-a" >
	    	...
	    	<div class="layui-col-md2 box-b">
	    		...
	    		<div class="box-d">
	    			<a class="ref-a" href="<%=request.getContextPath()%>/colletionInfo/form.do?museumId=${result.museumId}">
	    			<c:if test="${'1' eq level}">编辑资料</c:if>
	    			 <c:if test="${'1' ne level}">查看资料</c:if>
	    			</a>&nbsp;
	    	...
	    	<div class="layui-col-md2 box-b">
	    		...
	    		<div class="box-d">
	    			<a class="ref-a" href="<%=request.getContextPath()%>/museumPublicService/form.do?museumId=${result.museumId}">
	    			<c:if test="${'1' eq level}">编辑资料</c:if>
	    			<c:if test="${'1' ne level}">查看资料</c:if>
	    			</a>&nbsp;
	    ...
	    <c:if test="${'4' eq orgType}">
	    	...
	    	<div class="layui-col-md2 box-b">
	    		...
	    		<div class="box-c">
	    			<span class="span-one">机构人员情况</span>
	    		</div>
	    		<div class="box-c">
	    			<span class="span-two">资料完成度：</span>&nbsp;.
	    			<c:if test="${'1' eq result.housePer}"><span class="span-four">已完成</span></c:if>
	    		...
	    		<div class="box-d">
	    			<a class="ref-a" href="<%=request.getContextPath()%>/relicsBureauBuilding/list.do?relicsBureauId=${result.museumId}&relicsBureauName=${result.museumName}">
	    			  <c:if test="${'1' eq level}">编辑资料</c:if>
	    			 <c:if test="${'1' ne level}">查看资料</c:if>
	    			</a>&nbsp;
	    			<a class="ref-b"  href="javascript:editRecord('${result.museumId}','build','馆舍建筑情况修改记录');">修改记录</a>
	    	...
	    	<div class="layui-col-md2 box-b">
	    		...
	    		<div class="box-d">
	    			<a class="ref-b" href="<%=request.getContextPath()%>/relicsBureauPerson/goList.do?relicsBureauId=${result.museumId}">
	    			  <c:if test="${'1' eq level}">编辑资料</c:if>
	    			 <c:if test="${'1' ne level}">查看资料</c:if>
	    			</a>
	    		</div>
...
//          window.history.back(-1);  //关闭弹出层
    ...
    form.on('submit(saveDetail)', function(data){
    	var loading; 
    	$.ajax({
		       url:"<%=request.getContextPath()%>/museuminfo/saveDetail.do",
