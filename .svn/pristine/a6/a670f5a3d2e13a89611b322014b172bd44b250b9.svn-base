...
<form class="layui-form" id="newForm">
	...
	<input type="hidden" id="sequence" name="sequence" value="${banner.sequence }">
	<input type="hidden" id="carouselPositionId" name="carouselPositionId" value="${banner.carouselPositionId }">
	<input type="hidden" id="file_id" name="file_id" value="${banner.attchment.attId }">
	<input type="hidden" id="file_resultPath" name="file_resultPath" value="${banner.attchment.attPath }">
	<input type="hidden" id="file_realFileName" name="file_realFileName" value="${banner.attchment.attName }">
	<input type="hidden" id="file_isjunk" name="file_isjunk" value="${banner.attchment.attIsjunk }">
	<input type="hidden" id="file_size" name="file_size" value="${banner.attchment.attSize }">
	<input type="hidden" id="file_typeCode" name="file_typeCode" value="${banner.attchment.attFileType }">
	<div class="layui-form-item layui-row layui-col-xs12">
		<label class="layui-form-label">链接地址</label>
		<div class="layui-input-block">
			<input type="text" class="layui-input inputText" lay-verify="urlVerify" placeholder="这里输入链接地址" id="url" name="url" value="${banner.url }" style="width:300px">
		</div>
	</div>
	<div class="layui-form-item layui-row layui-col-xs12">
		...
		<div class="layui-input-block" style="width: 300px;">
			<select id="orgId" name="orgId" style="width: 300px;">
				...
				<c:forEach items="${orgList}" var="org" varStatus="row">
					<option value="${org.id}" <c:if test="${banner.orgId == org.id }">selected</c:if> >${org.name}</option>	
				</c:forEach>
			</select>
		</div>
	</div>
	<div class="layui-form-item layui-row layui-col-xs12">
		...
		<div class="layui-input-block">
			...
			<input type="hidden" id="pictureId" name="pictureId" value="${banner.pictureId }">
		</div>
	</div>
	<div class="layui-form-item layui-row layui-col-xs12">
		<div class="layui-input-block">
			<c:if test="${not empty banner.imgUrl }" var="imgUrlIsNotNull">
				<img id="iconImg" src="${banner.imgUrl }" style="border: 1px black solid;max-width: 300px;max-height: 300px;">
			</c:if>
			<c:if test="${!imgUrl }">
