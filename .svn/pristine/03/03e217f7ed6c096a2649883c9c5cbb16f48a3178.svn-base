...
<form class="layui-form" id="form">
	<div class="layui-form-item layui-row layui-col-xs12" style="padding-top:30px;">
		<label class="layui-form-label">旧部门名称</label>
		<div class="layui-input-block">
			<input type="text" class="layui-input inputText" placeholder="选择要修改的部门" id="parentname" readonly onclick="showTree();" style="width:300px"/>
            <input type="hidden" id="editId" name="id" value="${id }">
            <input type="hidden" id="museumId" name="museumId" value="${museumId }">
		</div>
	</div>
	<div class="layui-form-item layui-row layui-col-xs12">
		<label class="layui-form-label">新部门名称</label>
		<div class="layui-input-block">
...
layui.use(['form','layer'],function(){
    ...
    form.on("submit(save)",function(data){
    	//判断唯一名称是否已使用
    	if($("#editId").val() == ''){
    		layer.msg("您还没有选择要修改的部门");
    	}else if($("#name").val() == ''){
   			layer.msg("您还没有填写新的部门名称");
    	}else{
	    	$.ajax({
