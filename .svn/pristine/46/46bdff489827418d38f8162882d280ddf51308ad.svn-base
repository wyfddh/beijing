function Tag(inputId){
	...
	obj.initView=function(){
		...
		appendStr+='<div class="tagList"></div><div class="addTag"><input type="text" class="tagInput"/>';
		appendStr+='<i class="Hui-iconfont" style="font-size:22px;height:40px;line-height:40px;margin:5px;" id="addLabel">&#xe610;</i></div>';
		appendStr+='</div>';
		...
		if(!this.isDisable){
			...
			addLabel.click(function(){
			 		...
			 		for(var i=0;i<tags.length;i++){
			 			...
			 			tagsStr+=tags.eq(i).find("span").text()+end;
			 		}
			 		if(tags.length>3){
			 			$("#addLabel").hide();
			 			$(".tagInput").hide();
		...
		$("#"+inputId+"_tagcontaine .tagList .tagItem .delete").mousedown(function(){
			...
			var tags = $("#"+inputId+"_tagcontaine .tagList .tagItem");
	 		...
	 		if(tags.length>3){
	 			$("#addLabel").hide();
	 			$(".tagInput").hide();
//	 			$(".labelContainer").attr("width","400px");
	 		}else{
	 			$("#addLabel").show();
	 			$(".tagInput").show();
	 		}
