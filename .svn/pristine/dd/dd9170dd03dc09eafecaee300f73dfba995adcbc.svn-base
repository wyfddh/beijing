...
//        });
    ...
    function modifyPassword() {
    	var index = layui.layer.open({
            ...
            type : 2,
            area: ["500px", "400px"],
            content : [projectName + '/userManagemen/toModifyPassword.do','no'],
...
//        })
    ...
    function changeModel(model){
    	var _model = model;
    	$.ajax({
            url:projectName+"/back/changeModel.do",
            data:{model: _model},
            type:"POST",
            success:function(msg){
                if(msg.success == 1){
                	if(model == 'museum'){
                		...
                		layer.msg('已切换到文物修复资质单位类型');
                	}else if(model == 'scOrg'){
                		layer.msg('已切换到文物收藏单位类型');
                	}
                	setTimeout(function(){
                		location.reload();
    				},700);
    ...
    $(".zzOrg").on("click",function(){
    	changeModel("zzOrg");
    })
    $(".scOrg").on("click",function(){
    	changeModel("scOrg");
    })
