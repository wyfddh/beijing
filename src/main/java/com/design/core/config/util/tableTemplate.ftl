...
<hibernate-mapping>
	<class name="${entity.mtableName}" table="${entity.mtableName}" optimistic-lock="version">
				...
				<#else>
					<property name="${attr.fieldName}"
						...
						<column name="${attr.fieldName}" <#if attr.type=='double'||attr.type=='BigDecimal'>
							...
							<#if attr.fieldDefault?exists&&attr.fieldDefault!=''&&1==2>default="${attr.fieldDefault}"</#if>
