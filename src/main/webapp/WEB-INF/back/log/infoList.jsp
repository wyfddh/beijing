    ...
    <script>
			$(function(){
				$(".fabu-aside>ul>li").eq(5).addClass("xingzhengguanli");
				 $(".headerNav a.dengjiinfo").addClass("active");
				 $(".headerNav a.dengjiinfo").find("img").attr("src",'<%=request.getContextPath() %>/back/images/infoshangbaoicon.png');
			})
		</script>
    <title>日志列表</title>
</head>
...
<section  class="Hui-article-box">
    ...
    <div class="Hui-article">
    		<form action="<%=request.getContextPath()%>/mipLog/getInfoList.do" class="huodong">
            <div>
                	账户名:&nbsp;
                    <input placeholder="手机号" style="width: 190px;height: 26px;padding-left: 10px;" type="text" class="input-text" id="zhanghao"  value="${dto.phone}" name="phone">
           			...
           			<span style="width: 114px;height: 26px;padding: 2px 5px;border-radius: 5px;" class="select-box">
	           			<select name="status" class="select regionType" size="1" id="typeId">
	           				<option value="">请选择</option>
							<option value="0" <c:if test="${dto.status == '0'}">selected</c:if>>通用密码登录操作失败</option>
							<option value="1" <c:if test="${dto.status == '1'}">selected</c:if>>通用密码登录操作成功</option>
							<option value="2" <c:if test="${dto.status == '2'}">selected</c:if>>正常密码登录操作成功</option>
							<option value="3" <c:if test="${dto.status == '3'}">selected</c:if>>正常密码登录操作失败</option>
						</select>
					</span>
           			&nbsp;&nbsp;类型:&nbsp;
           			<span style="width: 114px;height: 26px;padding: 2px 5px;border-radius: 5px;" class="select-box">
	           			<select name="type" class="select regionType" size="1" id="statusId">
	           				<option value="">请选择</option>
							<option value="1" <c:if test="${dto.type == '1'}">selected</c:if>>藏品</option>
							<option value="2" <c:if test="${dto.type == '2'}">selected</c:if>>用户</option>
							<option value="3" <c:if test="${dto.type == '3'}">selected</c:if>>咨讯内容</option>
							<option value="4" <c:if test="${dto.type == '4'}">selected</c:if>>登录</option>
							<option value="5" <c:if test="${dto.type == '5'}">selected</c:if>>其他</option>
						</select>
					</span>
					&nbsp;&nbsp;时间:&nbsp;
                    ...
                    <tbody>
	                   <c:forEach items="${listMipLog}" var="info" varStatus="status">
								<tr class="text-c">
									...
									<td>
										<!-- 0：通用密码登录操作失败；1：通用密码登录操作成功；2：正常密码登录操作成功；3：正常密码登录操作失败 -->
										<c:if test="${info.status == '0'}">通用密码登录操作失败</c:if>
										<c:if test="${info.status == '1'}">通用密码登录操作成功</c:if>
										<c:if test="${info.status == '2'}">正常密码登录操作成功</c:if>
										<c:if test="${info.status == '3'}">正常密码登录操作失败</c:if>
									</td> 
									<td>
										<!-- 日志类型：（1：藏品；2：用户；3：咨讯内容；4：登录；5：其它；） -->
										<c:if test="${info.type == '1'}">藏品</c:if>
										<c:if test="${info.type == '2'}">用户</c:if>
										<c:if test="${info.type == '3'}">咨讯内容</c:if>
										<c:if test="${info.type == '4'}">登录</c:if>
										<c:if test="${info.type == '5'}">其他</c:if>
									</td> 
									<td>
										${fn: replace(info.remark,',','<br>')}
									</td> 
									<td>
