...
<form class="layui-form" id="form">
	<div class="layui-form-item layui-row layui-col-xs12" style="padding-top:30px;">
		...
		<div class="layui-input-block">
			<input type="text" class="layui-input inputText" placeholder="选择要删除的部门" id="parentname" readonly onclick="showTree();" style="width:300px"/>
            <input type="hidden" id="deleteId" name="id" value="${id }">
	...
	<div class="layui-form-item layui-row layui-col-xs12" style="margin-top:30px;">
		<div class="layui-input-block" style="padding-left:100px;">
			<button class="layui-btn layui-btn-sm layui-btn-danger" lay-submit lay-filter="delete">删除</button>
...
layui.use(['form','layer'],function(){
    ...
    form.on("submit(delete)",function(data){
    	//判断唯一名称是否已使用
    	if($("#deleteId").val() == ''){
   			layer.msg("您还没有选择要删除的部门");
    	}else{
    		layer.confirm("确认要删除吗，删除后不能恢复", { title: "删除确认" }, function (index) {
  		      layer.close(index);
  		      var id= $("#deleteId").val();
  		      var museumId = $("#museumId").val();
  		      $.post("<%=request.getContextPath() %>/museumPerson/deleteDep.do", { id: id,museumId:museumId  }, function (data) {
  		          if(data.success == 1){
  		          	layer.msg('删除成功');
		  		      setTimeout(function(){
		  					close();
