...
<form class="layui-form" id="newForm">
	<div class="layui-col-xs8" style="padding-right: 20px;">
		<div class="layui-form-item layui-row layui-col-xs12">
			<input type="hidden" id="id" name="id" value="${spreadtrum.id}">
			<input type="hidden" id="pictureId" name="pictureId" value="${spreadtrum.pictureId}">
			<input type="text" class="layui-input inputText" lay-verify="required" placeholder="请输入展览标题" id="headline" name="headline"  value="${spreadtrum.headline}" style="width:100%;">
			<script id="editorCh" type="text/plain" style="width:100%;" name="content">${spreadtrum.content}</script>			
		</div>
	...
	<div class="layui-col-xs4" style="padding-left: 20px;">
		...
		<div class="layui-form-item layui-row layui-col-xs12">
		<c:if test="${not empty spreadtrum.pictureId }" var="imgIsNotNull">
			<div id="showImg" style="width: 95%;height: 100px;margin-bottom: 10px;"><a onclick="deleteImg()" href="javascript:void(0);" title="点击删除"><img id="titleImgShow" style="max-height: 100%;max-width: 100%;" src="${spreadtrum.pictureId}" ></a></div>
		</c:if>
		<c:if test="${!imgIsNotNull }">
			<div id="showImg" style="width: 95%;height: 100px;display: none;margin-bottom: 10px;"><a onclick="deleteImg()" href="javascript:void(0);" title="点击删除"><img id="titleImgShow" style="max-height: 100%;max-width: 100%;" src="${spreadtrum.pictureId}" ></a></div>
		</c:if>
			<button type="button" class="layui-btn layui-btn-radius layui-btn-primary" id="uploadImg"><i class="layui-icon">&#xe67c;</i>上传标题图</button>
		</div>
		<div class="layui-form-item layui-row layui-col-xs12" >
			<label><b>展览类型</b></label>
		</div>
		<div class="layui-form-item layui-row layui-col-xs12">
		    <div class="layui-input-block" style="margin-left: 1px;">
		      <input id="radioOne" type="radio" name="exhibitionType" value="1" title="常设展览" lay-filter="type">
		      <input id="radioTwo" type="radio" name="exhibitionType" value="0" title="临时展览" lay-filter="type">
		    </div>
		</div>
		<div id="exhibitionTimediv">
			...
			<div class="layui-form-item layui-row layui-col-xs12" style="vertical-align:middle;">
				<div class="layui-input-inline">
					  <input type="text" class="layui-input" id="exhibitionTime" name="exhibitionTime" value="${spreadtrum.editor}" placeholder="选择展期 " style="width:300px;">
				 </div>
	        </div>
        </div>
         <div class="layui-form-item layui-row layui-col-xs12" style="margin-top: 10px;">
			<label><b>开展时间</b></label>
		</div>
		<div class="layui-form-item layui-row layui-col-xs12" style="vertical-align:middle;">
			<input type="text" class="layui-input inputText" lay-verify="required"  id="vipcurl" name="vipcurl" value="${spreadtrum.vipcurl}" style="width:300px;">
		</div>
		<div class="layui-form-item layui-row layui-col-xs12" style="margin-top: 10px;">
			<label><b>展览地址</b></label>
		</div>
		<div class="layui-form-item layui-row layui-col-xs12" style="vertical-align:middle;">
			<input type="text" class="layui-input inputText" lay-verify="required"  id="vimoveurl" name="vimoveurl" value="${spreadtrum.vimoveurl}" style="width:300px;">
		</div>			
	</div>
</form>
...
$(function(){
	...
	var exhibitionType = '${spreadtrum.exhibitionType}';
	$(":radio[name='exhibitionType'][value='" + exhibitionType + "']").attr("checked", "checked");
	if(exhibitionType=='1'){
		$("#exhibitionTimediv").hide();
	}
...
layui.use(['form','layer','laydate','upload'],function(){
    ...
    var saveFunction = function(){    	
    	$.ajax({
            url:"<%=request.getContextPath() %>/spreadtrum/update.do",
