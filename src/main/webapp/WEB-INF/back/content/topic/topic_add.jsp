...
<section  class="contentWrap">
    <form action="<%=request.getContextPath() %>/wenChuang/addAndUpdate.do" id="subForm" method="post">
        <div class="addTitle">
        	<label>专题名称：</label>
            <input type="text" name="name" placeholder="请输入专题名称" id="name">
        </div>
    ...
    $("#confirm").on("click",function(){
        ...
        $.ajax({
            ...
            success: function(data){
           		if(data.success == 1){
           			layer.confirm('保存成功，开始添加藏品吧？', {
        	            btn: ['开始添加藏品','以后再说'] //按钮
                	}, function(){
                		var url = "<%=request.getContextPath()%>/CollectionTopic/goCreateTopicPage.do?topicId="+data.data.id;
