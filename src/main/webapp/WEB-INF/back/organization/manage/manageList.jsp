...
<section  class="Hui-article-box">
    ...
    <div class="Hui-article">
        ...
        <form action="<%=request.getContextPath()%>/museumInfoManage/info.do" class="huodong" id="form">
        	<div class="hide">   
	        	每页显示条数:&nbsp; 
            ...
            <div>
                 <span id="shengji" class="inline">
                   ...
                   <span style="width: 114px;height: 26px;padding: 2px 5px;border-radius: 5px;" class="select-box">
                        <select name="area" class="select addType" size="1" style="style="width:100px" id="diqu">
						    <option value="">文物局</option>
						   <c:forEach items="${areaList}" var="a" varStatus="row">
...
<div id="layer111">
	<div id="addVideoIMG" class="addVideoIMG">
		<div class="body">
			<div class="one">
				<span id="shengji" class="inline">单位:&nbsp;
                   <span style="width: 120px;height: 26px;padding: 2px 5px;border-radius: 5px;" class="select-box">
                        <select name="area" class="select regionType" size="1" style="style="width:100px" id="addArea">
						    <option value="">文物局</option>
						   <c:forEach items="${areaList}" var="a" varStatus="row">
								<option value="${a.id}" <c:if test="${a.id eq dto.area}">selected</c:if> >${a.name}</option>
							</c:forEach>
                    ...
                    <span style="width: 120px;height: 26px;padding: 2px 5px;border-radius: 5px;" class="select-box">
                        <select name="unit" class="select" size="1" style="width:100px" id="addUnit">
						    <option value="">博物馆</option>
						    <c:forEach items="${unit}" var="u" varStatus="row">
								<option value="${u.id}" <c:if test="${u.id eq dto.unit}">selected</c:if> >${u.name}</option>
							</c:forEach>
...
$(function(){
	...
	$(".headerNav a.manage").addClass("active");
	$(".headerNav a.manage").find("img").attr("src",'<%=request.getContextPath() %>/back/images/bowuguanicon.png');
	...
	$("#uploadSave").on("click",function(){
		...
		if(urlPsd){
			if(imgUrl){
				...
				$.ajax({
					type:"post",
					url:"<%=request.getContextPath() %>/museumInfoManage/addInfo.do",
						dateType:"json",
						data:{type:type,unit:unit,imgUrl:imgUrl,urlPsd:urlPsd,area:area,fileName:fileName},
...
//	        		
	...
	function delect(id){
		$.ajax({
			type:"post",
			url:"<%=request.getContextPath() %>/museumInfoManage/delect.do",
				dateType:"json",
				data:{id:id},
				success:function(data){
					console.log(data);
