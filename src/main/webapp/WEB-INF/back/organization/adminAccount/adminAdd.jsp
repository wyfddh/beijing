...
<section>
    <!--<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 系统管理 <span class="c-gray en">&gt;</span> 新建管理员 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>-->
    <div class="addBwgUserbox">
        <form class="registerform" method="post" id="registForm" action="<%=request.getContextPath()%>/admin/user/addAdmin.do">
            <div class="containerBox">
                ...
                <div>
                    ...
                    <div class="info">
                        <div class="">
                            <label for="account"><span class="c-red">*</span>账号:</label>
                            <input style="width:216px;" type="text" id="account" name="phone" class=" input-text" placeholder="请输入11位手机号" datatype="m" errormsg="请输入11位手机号码！" ONBLUR="check()" onkeyup="keyup()"/>
                            &nbsp;&nbsp;&nbsp;
                            <label for="trueName" class=""><span class="c-red">*</span>真实姓名:</label>
                            <input style="width:216px;" type="text" id="trueName" name="userName" placeholder="请输入真实姓名，例如：张三" class="input-text refdis" datatype="s2-12"  nullmsg="请输入您的名字，例如：张三！" errormsg="名字至少2个字符,最多12个字符！" />
                        </div>
                        <div id="shengji">
                            <span class="c-red">*</span>单位:
                            	<span style="width: 114px;height: 26px;padding: 2px 5px;border-radius: 5px;" class="select-box">
	                           <select name="area" class="regionType select"  size="1" style="width:100px;" id="diqu">
                ...
                <div class="">
                    ...
                    <div class="" style="padding: 0">
                        <div style="margin-top: 20px;margin-bottom: 28px;" class="">
                        	<c:forEach items="${role}" var="u" varStatus="row">
							    <div class="radio-box"> 
                                    <input type="checkbox" value="${u.id}" name="roleId" class="refdis rolecheck">
                                    <label for="radio-1">${u.roleName}</label>
                                </div>
							</c:forEach>
                        </div>
                    </div>
				...
				<div class="">
					...
					<div style="margin-top: 20px;" class="">
						<div class="radio-box">
							<input type="radio" value="1" name="status" checked class="refdis">
							<label for="radio-13">启用</label>
						</div>
						<div class="radio-box" style="padding-left: 0">
							<input type="radio" value="0" name="status" class="refdis">
...
function check(){
   ...
   $.ajax({
     	url:"<%=request.getContextPath()%>/admin/user/checkAdmin.do",
       data:{phone:$("#account").val()},
       type:"POST",
       success:function(msg){
          if(msg == "ERROR1"){
        	  $("#submit").attr("disabled","disabled").removeClass("btn-primary").addClass("btn-default");
        	  layer.msg('该用户尚未注册，请注册后在添加为管理员。', {icon: 2});
        	  $("#submit").unbind("click");
        	  setTimeout(function(){
		            layer.closeAll();  //关闭弹出层
		        },2000);
    ...
    function submit(){
    	if($("#account").val() == ""){
            ...
            return false;
        }else if(!phoneNum.test($("#account").val())){
            layer.open({
                title:"提示",
                content:"账号必须为11位手机号码"
            });
            return false;
        }else if($.trim($("#trueName").val()) == ""){
            layer.open({
                title:"提示",
                content:"您还没有添加真实姓名"
            });
        	...
        	$.ajax({
    	     	url:"<%=request.getContextPath()%>/admin/user/addAdmin.do",
    	       data:$('#registForm').serialize(),
    	       type:"POST",
    	       success:function(msg){
    	          if(msg == "ERROR1"){
    	        	  layer.msg('您还没有添加真实姓名', {icon: 2});
    	        	  setTimeout(function(){
    			            layer.closeAll();  //关闭弹出层
    			        },2000);
    	          }else if(msg == "ERROR2"){
    	        	  layer.msg('您还没有选择单位或博物馆', {icon: 2});
    	        	  setTimeout(function(){
    			            layer.closeAll();  //关闭弹出层
    			        },2000);
    	          }else{
