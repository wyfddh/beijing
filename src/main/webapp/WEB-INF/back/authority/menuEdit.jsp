...
<form class="layui-form" id="newForm">
	...
	<div class="layui-form-item layui-row layui-col-xs12">
		...
		<div class="layui-input-block">
			<input type="text" class="layui-input inputText" lay-verify="required" placeholder="请输入菜单名称" id="menuname" name="menuname" value="${menu.menuname }" style="width:300px">
		</div>
	</div>
	<div class="layui-form-item layui-row layui-col-xs12">
		...
		<div class="layui-input-block" style="width:300px">
	      <select class="layui-select" name="type" id="type" lay-filter="type" lay-verify="required" style="width:300px">
              <option value="1" <c:if test="${menu.type == 1 }">selected</c:if> >菜单类型</option>
              <option value="2" <c:if test="${menu.type == 2 }">selected</c:if> >按钮类型</option>
	      </select>
	    </div>
	</div>
	<%-- <div class="layui-form-item layui-row layui-col-xs12 <c:if test="${menu.type != 2 }">layui-hide</c:if>" id="onlyString">
		<label class="layui-form-label">唯一标识</label>
		<div class="layui-input-block">
			<input type="text" class="layui-input inputText" placeholder="请输入唯一标识(英文字母)" id="roleids" name="roleids" value="${menu.roleids }" style="width:300px" onkeyup="this.value=this.value.replace(/[^a-zA-Z]/g,'')" readonly="readonly">
		</div>
	</div> --%>
	<div class="layui-form-item layui-row layui-col-xs12">
		<label class="layui-form-label">菜单链接</label>
		<div class="layui-input-block">
			<input type="text" class="layui-input inputText" placeholder="请输入菜单链接" id="menuurl" name="menuurl" value="${menu.menuurl }" style="width:300px">
		</div>
	</div>
	<div class="layui-form-item layui-row layui-col-xs12">
		<label class="layui-form-label">上级菜单</label>
		<div class="layui-input-block">
			<input type="text" class="layui-input inputText" lay-verify="required" placeholder="请选择上级菜单" id="parentname" value="${menu.parentname }" readonly onclick="showMenu();" style="width:300px"/>
	...
	<div class="layui-form-item layui-row layui-col-xs12">
		...
		<div class="layui-input-block">
			<input type="text" class="layui-input inputText" placeholder="请输入菜单排序，不填则默认为0" id="sequence" name="sequence" value="${menu.sequence }" style="width:300px">
		</div>
	</div>
	<div class="layui-form-item layui-row layui-col-xs12">
		<label class="layui-form-label">菜单图标</label>
		<div class="layui-input-block">
			<c:if test="${empty menu.iconremark }" var="iconisnull">
				<a class="layui-btn layui-btn-primary" id="iconSelect"><i class="layui-icon layui-icon-template-1"></i></a>
			</c:if>
			<c:if test="${!iconisnull }">
				<a class="layui-btn layui-btn-primary" id="iconSelect"><i class="${menu.iconremark }"></i></a>
			</c:if>
			<input type="text" class="layui-input inputText" placeholder="菜单图标样式，默认：layui-icon layui-icon-template-1" id="iconremark" name="iconremark" value="${menu.iconremark }" style="width:235px;display:inline-table;" readonly="readonly">
		</div>
	</div>
...
layui.use(['form','layer', 'iconpick'],function(){
    ...
    function valiOnlyName(){
    	...
    	$.ajax({
            ...
            data:{roleids:roleids, parentid:parentid, id:id},
            type:"POST",
            async: false,
    ...
    form.on("submit(save)",function(data){
    	if($("#menuname").val() == ""){
        	...
        	$.ajax({
	            url:"<%=request.getContextPath() %>/menu/editInfo.do",
