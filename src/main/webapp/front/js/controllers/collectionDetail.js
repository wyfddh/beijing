angular.module("collectionDetail-controller",[])
    .controller("CollectionDetail",function($scope,$rootScope,$http,$stateParams,$sce,$state){
...
(function($, window) {
	...
	$.fn.zoom = function(options) {
    	...
    	$http.get('../front/OCCollection/detail.do?id='+$stateParams.id).success(function(data){
    		$scope.detailData = data;
			$scope.collectionGsNo = data.data.mocid.mipOpenCulturalrelicInfo.gsNo;
    		console.log(data);

			$scope.success = data.data.mocid.isCollected;
			if($scope.success == 1){
    		...
    		$scope.name = data.data.mocid.mipOpenCulturalrelicInfo.name;
    		$scope.clickCounts = data.data.mocid.mipOpenCulturalrelicInfo.clickCounts;
    		$scope.lei = data.data.mocid.mipOpenCulturalrelicInfo.collectionsCategory;
    		$scope.chaodai = data.data.mocid.mipOpenCulturalrelicInfo.yearType;
    		$scope.level = data.data.mocid.mipOpenCulturalrelicInfo.collectionLevel;
    		$scope.danwei = data.data.mocid.mipOpenCulturalrelicInfo.collectionUnit;
    		$scope.inform = data.data.mocid.mipOpenCulturalrelicInfo.description;
    		$scope.pic = data.data.mocid.picture.thumb1;
    		$scope.bigPic = data.data.mocid.picture.url;
			...
			$scope.collectionSuccess = function(){
				...
				timeoutflagfn = setTimeout(function(){
					...
					$http({
						...
						params: {
							id: $stateParams.id,
							collectionType: data.data.mocid.mipOpenCulturalrelicInfo.collectionType
						}
			...
			if(data.data.mocid.mipOpenCulturalrelicInfo.threeDimensionalCollection==''){
				//$('.isThreeD').hide();
			...
			if(data.data.mocid.mipOpenCulturalrelicInfo.fVideo==''){
				//$('.isVideo').hide();
				$scope.isVideo = false;
			}else{
				//$('.isVideo').show();
			...
			if(data.data.mocid.mipOpenCulturalrelicInfo.fAudio==''){
				//$('.isAudio').hide();
				$scope.isAudio = false;
			}else{
				//$('.isAudio').show();
			...
			$scope.toThreeD = function(){
				window.open(data.data.mocid.mipOpenCulturalrelicInfo.threeDimensionalCollection);
			};
			...
			$scope.goDaolan = function(){
				$rootScope.daolanID = data.data.mocid.mipOpenCulturalrelicInfo.gsNo;
			};
			$scope.otherMofiList = function(index){
				$state.go('collectionDetail',{id:data.data.otherMofiList[index].mipOpenCulturalrelicInfo.id});
			};
			$scope.relations = function(index){
				$state.go('collectionDetail',{id:data.data.relations[index].mipOpenCulturalrelicInfo.id});
			};
			$scope.sce = $sce.trustAsResourceUrl;
			$scope.collectionAudio = data.data.mocid.mipOpenCulturalrelicInfo.fAudio;
			(function() {
			...
			jwplayer('mediaplayer').setup({
				...
				'image':data.data.mocid.picture.thumb3,
				'id': 'playerID',
				'width': '100%',
				'aspectratio':'1:1.76',
				autostart: true,
				'file': data.data.mocid.mipOpenCulturalrelicInfo.fVideo
			});
    	});
...
/*        	alert(document.documentElement.clientWidth);
            ...
            $("#imgCoverWrap img").addClass('noRotate');
            //如果宽大于高
