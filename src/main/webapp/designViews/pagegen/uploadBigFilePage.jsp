...
.selector-set .ss-item:hover i {
	...
	<div style="float:left;">
	 ...
	 <div style="width:100%;border: 1px solid #CCCCCC;" id="i_select_files_Complete">
		...
		&nbsp;&nbsp;上传完成&nbsp;(<font color="red">请上传文件</font>)</div>
...
function allComplete() {
		...
		$(".m_title2").each(function(index,obj){
			backFileJson.push({
				picName:""+$(obj).attr("picname")+"",
				fileName:""+$(obj).attr("filename")+"",
				fileId:""+$(obj).attr("fileid")+"",
				uploadTime:""+$(obj).attr("uploadtime")+"",
				fileUrl:""+$(obj).attr("fileurl")+""});
		});
		eval("parent.$.callbackBigFileFunction(backFileJson,'${designId}','${typecode}')");
	}
function guanbi() {
 ...
 */
 ...
 var config = {
			...
			browseFileBtn : "<span class=\"mybuttomStyel\" id='xuanzewenjian' href=\"#\" >添加文件</span>",/**  显示选择文件的样式, 默认: `<div>请选择文件</div>` */
...
//			swfURL : "/swf/FlashUploader.swf", /** SWF文件的位置 */
			...
			extFilters: [], /** 允许的文件扩展名, 默认: [] , ".rpm", ".rmvb", ".gz", ".rar", ".zip", ".avi", ".mkv", ".mp3"*/
...
//			onDestroy: function() {alert('onDestroy')} /** 文件上传出错的响应事件 */
	...
	function BigonComplete(file){
		...
		$.ajax({
			...
			data:"typecode=${typecode}&picName="+picName+"&mainId=${mainId}&detailId=${detailId}&fileCode="+fileMsg.fileName+"&fileName="+file.name,
			success:function (data1){
				...
				if(dataObj.msg=="0"){
					...
					var HtmlStr ="<div class='m_title2' picName='"+file.name+"' uploadtime='"+dataObj.obj.createDate+"' fileId='"+dataObj.obj.id+"' fileName='"+dataObj.obj.attachmenttitle+"' fileUrl='"+dataObj.obj.realpath+"' style='margin:1px;float:left;'>";
