...
//                    }
                  ...
                  //如果有三级菜单
					if(data[i].children[j].children !== undefined && data[i].children[j].children.length > 0){
						ulHtml += '<dl class="layui-nav-child layui-nav-child-2">';
						for(var k=0; k<data[i].children[j].children.length; k++){
							ulHtml += '<dd class="layui-nav-item layui-nav-item-2">';
							ulHtml += '	<a data-url="'+data[i].children[j].children[k].href+'">';
							ulHtml += '		<i class="seraph layui-icon '+data[i].children[j].children[k].icon+'"></i>&nbsp;<cite>'+data[i].children[j].children[k].title+'</cite></a>';
							ulHtml += '</dd>';
						}
						ulHtml += '</dl>';
					}
...
//                }
	...
	Tab.prototype.render = function() {
		...
		//默认打开工作台页面
		$("body .layui-nav-tree .layui-nav-item a:not('.mobileTopLevelMenus .layui-nav-item a')")[0].click();
	}
	...
	$(".closePageOther").on("click",function(){
		if($("#top_tabs li").length>2 && $("#top_tabs li.layui-this cite").text()!="工作台"){
			var menu = JSON.parse(window.sessionStorage.getItem("menu"));
		...
		}else if($("#top_tabs li.layui-this cite").text()=="工作台" && $("#top_tabs li").length>1){
			$("#top_tabs li").each(function(){
				if($(this).attr("lay-id") != '' && !$(this).hasClass("layui-this")){
					element.tabDelete("bodyTab",$(this).attr("lay-id")).init();
