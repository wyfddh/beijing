...
//			'<b class="stream-uploading-ico"></b>' +
		...
		'	<td  align="center" ><div class="stream-file-name"><strong></strong></div></td>#pic_typeHtml#'+
		'	<td style="width:40%;" align="center" ><div class="stream-cell-infos">' +
		...
		'</div></td>' +
		'	<td style="width:20%;" align="center" ><div class="stream-process">' +
		'	<span class="stream-cancel" onclick="javascript:void(0)" >删除</span>' +
...
//					'</div>' ,
		sTotalContainer = '<div id="#totalContainerId#" class="stream-total-tips">' +
					'	总进度：<span class="stream-process-bar"><span style="width: 0%;"></span></span>' +
					'	<span class="stream-percent">0%</span>，总上传<strong class="_stream-total-uploaded">&nbsp;</strong>' +
					'	，总文件大小<strong class="_stream-total-size">&nbsp;</strong>' +
					'</div>',
		sTopContainer = '<table id="#topContainerId#" style="width:100%;border: 1px solid #ccc;background-color: #FFFFE1;"> <tr>' +
					'	<td  align="center">文件名</td>#pictdHtml#'+
					'	<td style="width:40%;" align="center">进度</td>' +
			...
			switch(event.type){
				case "load":
					...
					if(uploaded < this.get("size") -1) {
						...
						if(respJson.message=="409"||respJson.message=="401"){
							fShowMessage("上传数据连接中断，正在恢复连接.......");
							_t.refresh__();
							fShowMessage("连接已恢复，正在上传。");
						}else{
	...
	function Main(cfg){
		...
		this.config = {
			enabled : !0,
			picTypes:fIsArray(cfg.picTypes) ? cfg.picTypes : [],
	...
	Main.prototype = {
		...
		initializer : function() {
			...
			picTypes = fIsArray(this.config.picTypes) ? this.config.picTypes : [];
			this.startPanel = this.config.browser || document.getElementById(this.config.browseFileId);
			/** the default UI */
			if(picTypes.length>0){
				pictdValueHtml+='	<td style="width:10%;" align="center" >' +
				'<select id="pic_type" style="width:80px;" mame="pic_type">'+
				'<option value="--">请选择</option>';
				for(var i=0;i<picTypes.length;i++){
					pictdValueHtml+='<option value=\''+picTypes[i].codeValue+'\'>'+picTypes[i].codeName+'</option>';
				}
				pictdValueHtml+='</select>';
				pictdValueHtml+='</td>';
			}
			
			if(picTypes.length>0){
				pictdHtml+='	<td style="width:15%;" align="center"><font color=red>*</font>拍摄角度</td>';
			}
			if (!this.config.customered) {
				if(picTypes.length>0){
					sTopContainer=sTopContainer.replace("#pictdHtml#", pictdHtml);
					sCellFileTemplate=sCellFileTemplate.replace("#pic_typeHtml#", pictdValueHtml);
				}else{
					sTopContainer=sTopContainer.replace("#pictdHtml#", "");
					sCellFileTemplate=sCellFileTemplate.replace("#pic_typeHtml#", "");
				}
				...
				var filesContainerId = fGenerateId("files-container"),
					topContainerId = fGenerateId("top-container");
				...
				var filesQueue = fCreateContentEle(sFilesContainer.replace("#filesContainerId#", filesContainerId).replace("#filesQueueHeight#", this.config.filesQueueHeight)),
					topQueue = fCreateContentEle(sTopContainer.replace("#topContainerId#", topContainerId)),
					totalQueue = fCreateContentEle(sTotalContainer.replace("#totalContainerId#", totalContainerId));
				filesQueuePanel.appendChild(topQueue);
		...
		onSelect : function(list) {
			fShowMessage("上传附件个数: " + list.length+"个");
		},
		onFileCountExceed : function(selected, limit) {
			fShowMessage("文件数量:" + selected + ", 但有限的:" + limit, true);
		},
		onMaxSizeExceed : function(file) {
			fShowMessage("文件:" + file.name + " 大小是:" + file.size +" 超过限制:" + file.limitSize, true);
		},
		onExtNameMismatch: function(file) {
			fShowMessage("允许ext的名字: [" + file.filters.toString() + "], 不是为了 " + file.name, true);
		},
		onAddTask: function(file) {
			fShowMessage("添加到任务 << 名称: [" + file.name);
		},
		onCancel : function(file) {
			fShowMessage("取消上传文件: " + file.name);
		},
		onStop : function() {
			fShowMessage("停止!");
		},
		onCancelAll : function(numbers) {
			fShowMessage(numbers + " 文件取消上传! ");
		},
		onComplete : function(file) {
			fShowMessage("附件【" + file.name + "】 上传成功 <img src=\"/plug-in/stream/css/img/u860.png\"></img>");// 大小:" + file.size + "
		},
		onQueueComplete : function(msg) {
			//fShowMessage("onQueueComplete(msg: "+msg+")	---==>		[OK]");
		},
		onUploadError : function(status, msg) {
			fShowMessage("发生错误,状态:" + status + ", 消息: " + msg, true);
		},
		disable : function() {
		...
		upload : function(index) {
			...
			var picName="";
			if(picTypes.length>0){
				var s111 = document.getElementById("i_stream_files_queue").getElementsByTagName("select");
					var index11=s111[0].selectedIndex;
					var selectedValue=s111[0].options[index11].value;
					var selectedName=s111[0].options[index11].text;
					picCode=selectedValue;
					picName=selectedName;
			}
			...
			var postVarsPerFileJson=this.get("postVarsPerFile");
			postVarsPerFileJson.picCode=picCode;
			postVarsPerFileJson.picName=picName;
			vars = fMergeJson(vars, postVarsPerFileJson);
