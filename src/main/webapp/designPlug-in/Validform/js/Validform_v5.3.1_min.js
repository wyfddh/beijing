(function($, win, undef) {
	...
	Validform.util = {
		...
		check : function(curform, brothers, subpost, bool) {
			...
			//字段重复校验,转换成大写进行验证
			var  upperValidType=$(this).attr("upperValidType");
			//var upptipType=settings.tiptype;
			//if(tipType==null||tipType!=1){
			if(upperValidType!=null){
		       var params=upperValidType.split(",");
		       var value___= $(this).val();
		       var upperValue=value___.toUpperCase();//alert(upperValue);
		       var ajaxResultValue=Validform.util.ajax_check(params[0],params[1],upperValue,$("input[name='"+params[2]+"']").val());
			   var resultParams= new Array(); //定义一数组
                resultParams=ajaxResultValue.split("+"); //字符分割     
			   if (resultParams[1] == "false" && tipType == 1) {
                    msgobj.find(".Validform_info").html(resultParams[0]);
		...
		submitForm : function(settings, flg, url, ajaxPost, sync) {
			...
			//var tipType=settings.tiptype
					//字段重复校验
			if(tipType==null||tipType!=1){
				 curform.find("[upperValidType]").each(function(obj){
				   var upperValidType=$(this).attr("upperValidType");
					if(upperValidType!=null&&$(this).val()!=""){
				       var params=upperValidType.split(",");
				       var value___= $(this).val();
				       var upperValue=value___.toUpperCase();//alert(upperValue);
				       var ajaxResultValue=Validform.util.ajax_check(params[0],params[1],upperValue,$("input[name='"+params[2]+"']").val());
	...
	Validform.prototype = {
		...
		submitFormExt__ : function(flag, url) {
			var obj = this;
			var subflag = Validform.util.submitForm.call($(obj.forms[0]),
					obj.settings, flag, url);
			subflag === undef && (subflag = true);
